...
export function EngineCard({name, code, description, status='Operational', onClick}) { const rephrase=name.toLowerCase()==='rephrase'; const signals=[['Risk','0.12'],['Drift','0.08'],['Align','0.94'],['Level','1'],['Intent','yes']]; return <article className={'engine-card engine-'+name.toLowerCase().replace(/[^a-z0-9]+/g,'-')} onClick={onClick} tabIndex={onClick?0:undefined}><span className="engine-scan"/><div className="engine-top"><span className="engine-code">{code}</span><span className="status-dot">● {rephrase?'Online':status}</span></div><h3>{name}</h3>{rephrase&&<span className="engine-category">Expressive Intelligence</span>}<p>{description}</p>{rephrase&&<><div className="engine-signals">{signals.map(([label,value])=><span key={label} className="engine-signal"><small>{label}</small><strong>{value}</strong></span>)}</div><div className="engine-preview"><small>Input → Output</small><span>“Why didn’t you do this right?”</span><span>“Could you help me understand what happened here?”</span></div></>}<span className="text-link">Inspect engine →</span></article>; }
