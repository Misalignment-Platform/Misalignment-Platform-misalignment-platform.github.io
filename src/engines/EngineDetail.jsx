...
export function EngineDetail({ name }) {
  ...
  return <section className="section engine-detail">
    ...
    {['sentinel','omnitrax','ata-cont','codex'].includes(name) && <Card title="Engine signals"><BankGrid items={data.signals.map(signal => ({ key: signal.name, value: signal.value, note: 'mock signal' }))}/><p className="eyebrow">Signals are deterministic demo evidence pending live engine integration.</p></Card>}
